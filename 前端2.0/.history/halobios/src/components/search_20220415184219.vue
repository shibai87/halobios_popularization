    ...
    <van-pull-refresh v-model="isLoading" @refresh="onRefresh">
      ...
      <van-row>
        <van-col span="8" offset="1">
          <van-dropdown-menu>
            <van-dropdown-item v-model="options1_value" :options="options1" />
          </van-dropdown-menu>
        </van-col>
        <van-col span="8">
          <van-button @click="showPopup" size="medium">请选择搜索的标题
          </van-button>
        </van-col>
    ...
    methods: {
      ...
      onConfirm(value) {
        ...
        for (let i = 0; i < this.infos.length; i++) {
          if (this.options1_value == 0 && this.infos[i].knowledge_title == value) {
            this.search_value = this.infos[i].knowledge_id
          }
          if (this.options1_value == 1 && this.infos[i].activity_title == value) {
            this.search_value = this.infos[i].activity_id
          }
          if (this.options1_value == 2 && this.infos[i].video_title == value) {
            this.search_value = this.infos[i].video_id
          }
      ...
      getsearch() {
        if(this.search_value){
          ...
          // 用户从个人中心跳转至知识详情时需要
          sessionStorage.setItem('source_id', this.search_value)
          this.$router.push({
            path: '/knowledge_details',
            query: {
              knowledge_id: this.search_value
            }
          })
        } else if (this.options1_value == 1) {
          ...
          this.$router.push({
            ...
            query: {
              classroom_id: this.search_value
            }
          })
        }
        }else{
