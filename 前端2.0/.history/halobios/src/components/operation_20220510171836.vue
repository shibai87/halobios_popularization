    ...
    export default {
        ...
        methods: {
            getOperadionNumber() {
                ...
                OperationService.getOperationNumber(this.getOperationSourceType, this.getSourceId, 1).then((res) => {
                    this.StarNum = res
                })
                OperationService.getOperationNumber(this.getOperationSourceType, this.getSourceId, 2).then((res) => {
                    this.CommentNum = res
                })
            ...
            getUserOperation() {
                OperationService.getUserOperation(this.getUserId, this.getOperationSourceType, this.getSourceId, 0)
                    .then((res) => {
                        if (res.msg == "用户已有操作") {
                ...
                OperationService.getUserOperation(this.getUserId, this.getOperationSourceType, this.getSourceId, 1)
                    .then((res) => {
                        if (res.msg == "用户已有操作") {
            ...
            change(val) {
                console.log(this.getJudgeLogin)
                if (this.getJudgeLogin) {
                    if (val == 0) {
                        // 取消操作
                        ...
                        // 添加操作
                            ...
                            OperationService.addUserOperation(this.getUserId, this.getOperationSourceType, this
                                .getSourceId, 0).then((res) => {
                                console.log(res)
                                if (res.msg == '添加成功') {
                                    Dialog.alert({
                                        message: '点赞成功!',
