        ...
        if (this.options1_value == 0) {
            KnowledgeService.search().then((res) => {
              ...
              if(this.infos.length==0){
                ...
                this.columns.push(res[i].knowledge_title)
              }
              }
            ...
            ActivityService.search().then((res) => {
               console.log(res.length)
              this.infos = res
                if(this.infos.length==0){
                  Dialog.alert({
                     title:'查询科普保护活动模块内容标题',
                  message: '该模块无资源！请选择其他模块！',
          })
              }else{
                this.show = true;
                 for (let i = 0; i < res.length; i++) {
                this.columns.push(res[i].activity_title)
              }
                }
             
            });
        } else {
            VideoService.search().then((res) => {
              this.infos = res
                if(this.infos.length==0){
                  Dialog.alert({
                    title:'查询科普小课堂模块内容标题',
               ...
               for (let i = 0; i < res.length; i++) {
                this.columns.push(res[i].video_title)
              }}
