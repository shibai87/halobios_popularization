...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            judgeComment() {
                CommentService.judgeComment(this.operation_user_id,this.operation_source_type,this.operation_source_id).then((res) => {
                    // console.log(res)
                        if (res.msg == '用户已有过该操作') {
                            this.my_judge_comment = true
                            this.my_message = res.comment_content
                            this.my_old_message = this.my_message
                            this.my_report = res.comment_state
            ...
            addComment() {
                // this.axios.post(
                //         "http://192.168.197.65:8083/operation/addComment?operation_user_id=" + this.getUserId +
                //         "&operation_source_type=" + this.getOperationSourceType + "&operation_source_id=" + this
                //         .getSourceId +
                //         "&comment_content=" + this.my_message
                //     )
                    CommentService.addComment(this.operation_user_id,this.operation_source_type,this.operation_source_id,this.comment_content).then((res) => {
                        console.log(res)
            ...
            updateComment() {
                ...
                if (this.my_message) {
                    if (this.my_old_message == this.my_message) {
                        ...
                        this.axios.get("http://192.168.197.65:8083/operation/updateComment?operation_id=" + this
                                .operation_id +
                                "&comment_content=" + this.my_message)
                            .then((res) => {
            ...
            deleteComment() {
                Dialog.alert({
                    ...
                    this.axios.get("http://192.168.197.65:8083/operation/deleteComment?operation_id=" + this
                            .operation_id)
                        .then((res) => {
            ...
            getloadComment() {
                if (this.getJudgeLogin) {
                    this.judgeComment()
                    this.axios.get("http://192.168.197.65:8083/operation/getCommentList?operation_source_type=" + this
                            .getOperationSourceType +
                            "&operation_source_id=" + this.getSourceId + "&operation_user_id=" + this.getUserId)
                        .then((res) => {
                    ...
                    this.axios.get("http://192.168.197.65:8083/operation/getCommentList?operation_source_type=" + this
                            .getOperationSourceType +
                            "&operation_source_id=" + this.getSourceId + "&operation_user_id=" + "")
                        .then((res) => {
                            this.commentList = res.data
                        })
                }
            ...
            // 举报
            report(comment_id) {
                this.axios.get("http://192.168.197.65:8083/report/add?operation_id=" + this.operation_id +
                        "&comment_id=" + comment_id + "&report_user_id=" + this.getUserId)
                    .then((res) => {
                        // console.log(res)
                        if (res.data.msg == '举报成功') {
                            Dialog.alert({
                                message: '举报成功!',
                            }).then(() => {
                                this.reload()
