        ...
        <van-pull-refresh v-model="isLoading" @refresh="onRefresh">
            <div class="comment_other" v-if="commentList.length">
                <div v-for="(item,index) in commentList" :key="index">
                    <van-row>
                        <van-notice-bar v-if="item.comment_state==1" :scrollable="false" text="您的评论已被举报，正在审核中，您可选择删除评论！" />
                        <van-notice-bar v-else-if="item.comment_state==2" scrollable text="您的评论已被举报，审核未通过，已设置该评论对其他用户不可见！您可选择删除评论！" />
                        ...
                        <van-col span="5" offset="2" class="comment_other_operation">
                            ...
                            <van-button size="small" type="warning" @click="deleteComment" v-if="item.comment_state!=0">删除</van-button>
                        </van-col>
                    </van-row>
                </div>
            </div>
    ...
    export default {
        ...
        methods: {
            ...
            getloadComment() {
                ...
                this.axios.get("http://192.168.1.105:8083/operation/getUserCommentList?operation_user_id="+this.getUserId)
                    .then((res) => {
