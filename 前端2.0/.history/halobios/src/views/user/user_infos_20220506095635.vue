  ...
  <div id="user_infos">
    ...
    <van-pull-refresh v-model="isLoading" @refresh="onRefresh">
      <van-form @submit="onSubmit">
        <van-field v-model="this.info[0].user_name" label="用户名" placeholder="请输入用户名" class="name" clearable />
        <van-field v-model="this.info[0].user_tel" type="tel" label="手机号" placeholder="请输入手机号码" disabled />
        <van-field v-model="this.info[0].user_pwd" type="password" label="密码" clearable />
        <van-row class="sex">
          <van-col span="6">
            <div class="sex_title">性别</div>
          </van-col>
          <van-col span="18">
            <van-radio-group v-model="this.info[0].user_sex" direction="horizontal">
    ...
    methods: {
      ...
      getloadInfo() {
          UserService.getloadInfo(this.getUserId).then((res) => {
            this.info.push(res)
             console.log(res,this.info[0].user_name)
          })
      },
      // 刷新
      onRefresh() {
