...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 获取知识列表+查询列表
      getLoadactivityList() {
        this.axios.get("http://192.168.1.105:8083/activity/get_activity_listByUserId?sign_up_user_id=" + this.getUserId)
          .then((res) => {
...
<style>
   /* user_infos_head */
    #user_activity  .van-nav-bar__content {
        height: 70px;

    }

    #user_activity .van-nav-bar__title {
        height: 30px;
        line-height: 30px;
        font-size: 25px;
    ...
    #user_activity .van-icon:before {
        color: #000;
        font-size: 35px;
    }
  /*  */
  #user_activity .van-button {
    display: block;
    width: 250px;
    margin: 5px auto 0;
  }
