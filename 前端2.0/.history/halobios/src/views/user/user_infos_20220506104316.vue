    ...
    methods: {
      ...
      getloadInfo() {
           UserService.getloadInfo(this.getUserId).then((res) => {
            this.info=res
            this.info.user_sex=JSON.stringify(this.info.user_sex)
      ...
      onSubmit() {
        this.info.user_sex=JSON.parse(this.info.user_sex);
        console.log(typeof this.info.user_sex,this.info.user_sex)
         console.log(this.info.user_name,this.info.user_pwd,this.info.user_sex,this.info.getUserId)
        // this.axios.get("http://192.168.1.105:8083/user/updateuser?user_name="+this.user_name+"&user_pwd="+this.user_pwd+"&user_sex="+this.user_sex+"&user_id="+this.getUserId)
             UserService.updateuser(this.info.user_name,this.info.user_pwd,this.info.user_sex,this.getUserId).then((res) => {
            console.log(res)
           if (res.msg == '修改成功') {
              Dialog.alert({
                title: '修改成功',
