    ...
    export default {
        ...
        methods: {
            ...
            getloadOperation() {
                ...
                OperationService.getloadOperation(this.getUserId,this.active,operation_type).then((res) => {this.List = res.data})
            },
             user_operation_detail(operation_source_id) {
                if (this.active== 0) {
                    // 科普知识
                        KnowledgeService.getKnowledge(operation_source_id).then((res) => {
                            console.log(res[0])
                ...
                // 科普活动
                if (this.active== 1) {
                        ActivityService.getactivity(operation_source_id).then((res) => {
                ...
                // 科普小课堂
                if (this.active== 2) {      
                        VideoService.getclassroom(operation_source_id).then((res) => {
...
<style lang="less">
      ...
      #user_other_operation   .van-icon:before {
       ...
       #user_other_operation  .van-button {
        ...
        #user_other_operation .van-empty{
            top: 100px;
            position: absolute;
        }
</style>
