...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 获取需编辑的知识详情
            getLoadactivity() {
                // this.axios.get("http://192.168.197.65:8083/activity/get_activity?activity_id=" + this.getActivityId)
                    ActivityService.getLoadactivity(this.getActivityId).then((res) => {
                ...
                console.log("1", this.getJudgeLogin)
                if (this.getJudgeLogin) {
                    console.log("2", this.getUserId, "3", this.getActivityId)
                    this.axios.get("http://192.168.197.65:8083/activity/get_SignUp?sign_up_user_id=" + this.getUserId +
                            "&sign_up_activity_id=" + this.getActivityId)
                        .then((res) => {
                            console.log("4", res.data.data)
                            this.judge_sign_up = res.data.data
                        })
                }
            },

            Video() {
                var url = "http://192.168.197.65:8083/activity/" + this.ruleForm.activity_file_name
                console.log(url)
                return url
            },
            addSignUp(activity_id) {
                this.axios.get("http://192.168.197.65:8083/activity/add_SignUp?sign_up_user_id=" + this.getUserId +
                        "&sign_up_activity_id=" + activity_id)
                    .then((res) => {
            ...
            updataSignUp() {
                this.axios.get("http://192.168.197.65:8083/activity/updata_SignUp?sign_up_id=" + this.judge_sign_up)
                    .then((res) => {
                        console.log(res.data)
