...
<script>
    ...
    export default {
        ...
        watch:{
            value(){
                 this.$axios.get("http://192.168.1.105:8083/operation/getAllCommentListByCommentId?operation_source_type=" + this
                        .getOperationSourceType +
                        "&operation_source_id=" + this.getSourceId+"&comment_id="+this.value)
                    .then((res) => {
                        console.log(res.data)
                        this.commentList = res.data
                        for (let i = 0; i < this.commentList.length; i++) {
