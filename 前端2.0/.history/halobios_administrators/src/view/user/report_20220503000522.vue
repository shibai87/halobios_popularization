        ...
        this.$axios.get("http://192.168.1.105:8083/report/getBycommentId?comment_id="+this.$route.query.comment_id)
          .then((res) => {
            ...
            this.report = res.data
                            if (this.report.comment_state == 1) {
                                this.report.comment_state = "评论已通过"
                            } else
                            if (this.report.comment_state == 2) {
                                this.report.comment_state = "评论待审核"
                            } else
                            if (this.report.comment_state == 3) {
                                this.report.comment_state = "该评论未通过审核"
                            }
          })
      },
      submit(comment_id){
