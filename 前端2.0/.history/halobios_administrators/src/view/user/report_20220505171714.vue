...
<script>
  export default {
    ...
    methods: {
      ...
      getLoadreport() {
        // alter(this.$route.query.comment_id)
        this.$axios.get("http://192.168.197.65:8083/report/getBycommentId?comment_id="+this.$route.query.comment_id)
          .then((res) => {
            console.log(res.data[0])
            this.report = res.data
                if (this.report[0].comment_state == 1) {
                    this.report[0].comment_state = "评论已通过"
                } else
                if (this.report[0].comment_state == 2) {
                    this.report[0].comment_state = "评论待审核"
                } else
                if (this.report[0].comment_state == 3) {
                    this.report[0].comment_state = "该评论未通过审核"
                }
          })
      },
      submit(comment_id){
        ...
        if(this.form.comment_state){
          this.$axios.post("http://192.168.197.65:8083/report/updata?comment_state="+this.form.comment_state+"&comment_id="+comment_id+"&report_result="+this.form.report_result)
          .then((res)=>{
