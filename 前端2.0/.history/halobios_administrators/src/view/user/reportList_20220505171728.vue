...
<script>
  export default {
    ...
    methods: {
      ...
      getLoadreportList() {
        this.$axios.get("http://192.168.197.65:8083/report/get")
          .then((res) => {
      ...
      submit(comment_id) {
        ...
        if (this.form.comment_state) {
          this.$axios.post("http://192.168.197.65:8083/report/updata?comment_state=" + this.form.comment_state +
      ...
      seachBySourceTypeAndCommentState() {
        this.$axios.get("http://192.168.197.65:8083/report/getBySourceTypeAndCommentState?operation_source_type=" + this
            .source_type + "&comment_state=" + this.comment_state)
          .then((res) => {
            console.log(res.data)
            this.reportList = res.data
            for (let i = 0; i < this.reportList.length; i++) {
    ...
    watch: {
      comment_state() {
        if (!this.source_type) {
          console.log()
          this.$axios.get("http://192.168.197.65:8083/report/getByCommentState?comment_state=" + this.comment_state)
            .then((res) => {
              console.log(res.data)
              this.reportList = res.data
              for (let i = 0; i < this.reportList.length; i++) {
      ...
      source_type() {
        if (!this.comment_state) {
          this.$axios.get("http://192.168.197.65:8083/report/getBySourceType?operation_source_type=" + this.source_type)
            .then((res) => {
              console.log(res.data)
              this.reportList = res.data
              for (let i = 0; i < this.reportList.length; i++) {
