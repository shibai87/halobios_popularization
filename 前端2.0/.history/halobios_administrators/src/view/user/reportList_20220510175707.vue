<template>
  <div id="reportList">
    ...
    <div v-else>
      <el-empty description="没有需审核的举报！" v-show="this.comment_state==null&&this.source_type==null"></el-empty>
      <el-empty description="无该审核状态下的评论！" v-show="this.comment_state!=null"></el-empty>
      <el-empty description="无该类型下的评论！" v-show="this.source_type!=null"></el-empty>
    </div>
  </div>
</template>
    ...
    methods: {
      ...
      getLoadreportList() {
          ReportService.get().then((res) => {
            console.log(this.comment_state,this.source_type)
            this.reportList = res
