<template>
  <div id="reportList">
    <div  class="reportList_select">
         <el-row>
  <el-col :span="6"><div class="reportList_select_by_comment_state reportList_select_title">评论查询:</div></el-col>
  <el-col :span="6"> <el-select v-model="comment_state" placeholder="请选择要查询的审核状态" clearable>
        <el-option v-for="item in comment_state_options" :key="item.value" :label="item.label" :value="item.value">
        </el-option>
      </el-select></el-col>
   <el-col :span="6"> <div class="reportList_select_by_source_type reportList_select_title">评论查询:</div></el-col>
  <el-col :span="6"><el-select v-model="source_type" placeholder="请选择要查询的资源状态" clearable>
          ...
          <li v-for="(item,index) in reportList" :key="index" class="infinite-list-item">
            ...
            <div>
              <el-button type="primary" @click="dialogFormVisible = true">编辑</el-button>
               <el-button type="primary" @click="dialogFormVisible = true">查看资源详情</el-button>
            </div>
  ...
  export default {
    ...
    methods: {
      ...
      seachBySourceTypeAndCommentState(){
         this.$axios.get("http://192.168.1.105:8083/report/getBySourceTypeAndCommentState?operation_source_type="+this.source_type+"&comment_state="+this.comment_state)
                     .then((res) => {
                      console.log(res.data)
                      this.reportList = res.data
                      for (let i = 0; i < this.reportList.length; i++) {
