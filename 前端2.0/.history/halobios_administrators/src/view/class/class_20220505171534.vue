...
<script>
  ...
  export default {
    ...
    methods: {
      // 获取知识列表+查询列表
      getLoadclassList() {
        this.axios.get("http://192.168.197.65:8083/video/get_video_list?video_upload=" + 1)
          .then((res) => {
      ...
      delete_class(video_id, video_file_name) {
         ...
         this.axios.get("http://192.168.197.65:8083/video/delete_video?video_id=" + video_id + "&video_file_name=" +
            video_file_name)
          .then((res) => {
      ...
      //视频路径
      Video(row) {
        var url = "http://192.168.197.65:8083/video/" + row.video_file_name
        console.log(url)
        return url
      },
    },
