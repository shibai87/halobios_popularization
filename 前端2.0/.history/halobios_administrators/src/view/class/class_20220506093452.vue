...
<script>
  ...
  export default {
    ...
    methods: {
      // 获取知识列表+查询列表
      getLoadclassList() {
        this.axios.get("http://192.168.1.105:8083/video/get_video_list?video_upload=" + 1)
          .then((res) => {
      ...
      delete_class(video_id, video_file_name) {
         ...
         this.axios.get("http://192.168.1.105:8083/video/delete_video?video_id=" + video_id + "&video_file_name=" +
            video_file_name)
          .then((res) => {
