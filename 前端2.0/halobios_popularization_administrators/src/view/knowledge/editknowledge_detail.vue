    ...
    <div id="editKnowledge_detail">
        ...
        <el-main>
            <el-form :model="ruleForm"  ref="ruleForm" label-width="220px" class="demo-ruleForm">
                ...
                <el-form-item label="(已上传的)科普知识所属分类名" v-if="this.ruleForm.knowledge_sort_id!=0">
                    <div class="editKnowledge_sort">
                    {{sort.sort_type_1}}->{{sort.sort_type_2}}->{{sort.sort_type_3}}->{{sort.sort_type_4}}->{{sort.sort_type_5}}->{{sort.sort_type_6}} 
                    </div>
                </el-form-item>
...
import Constant from "../../assets/constant.js"
    export default {
        ...
        methods: {
            ...
            // 获取需编辑的知识详情
              getLoadKnowledge() {
                    KnowledgeService.get_knowledge(this.getKnowledge).then((res) => {
                        console.log(res[0])
                        this.ruleForm=res[0]
                        this.ruleForm.old_img_name=this.old_img_name;
                        this.ruleForm.old_knowledge_sort_id=this.ruleForm.knowledge_sort_id
                        if(this.ruleForm.old_knowledge_sort_id!=0){
                            this.getSort()
                        }
                    });
            },
            getSort(){
                if(this.ruleForm.knowledge_sort_id!=null){
                    KnowledgeService.get_sort(this.ruleForm.knowledge_sort_id).then((res) => {this.sort=res[0]})
                }
            },
            Image() {
                this.old_img_name=this.ruleForm.knowledge_img;
                var url = Constant.URL+"/knowledge/" +this.ruleForm.knowledge_img
                return url
            },
            // 提交
             submitForm() {
                 this.action=Constant.URL+"/knowledge/updatasort"
                 if(this.getSortListLastId){
                     this.ruleForm.knowledge_sort_id = this.getSortListLastId
                 }else{
                     this.ruleForm.knowledge_sort_id=this.ruleForm.old_knowledge_sort_id
                 }
                
                this.ruleForm.knowledge_upload = 1
                 console.log(this.getSortListLastId)
                console.log(this.ruleForm.knowledge_sort_id)
                if (this.ruleForm.knowledge_title && this.ruleForm.knowledge_sort_id!=0 && this.ruleForm
                    .knowledge_content && this.knowledge_img_name) {
                        KnowledgeService.get_knowledgeTitle(this.ruleForm.knowledge_title).then((res) => {
                            if (res.msg == '查询成功'&&res.data!=this.getKnowledge) {
                                this.$message({
            ...
            keepForm() {
                this.action=Constant.URL+"/knowledge/updatasort"
                this.ruleForm.knowledge_sort_id=Number(this.ruleForm.knowledge_sort_id);
                if(this.getSortListLastId){
                     this.ruleForm.knowledge_sort_id = this.getSortListLastId
                }
