      ...
      <div v-if="this.reportList.length">
        <ul class="infinite-list" v-infinite-scroll="load" style="overflow:auto">
          <li v-for="(item,index) in reportList" :key="index" class="infinite-list-item">
            ...
            <div>
              <el-button type="primary" @click="dialogFormVisible = true,comment_id_change=item.comment_id">编辑</el-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      submit() {
        if (this.form.comment_state1) {
          ReportService.updata(this.form.comment_state1,this.comment_id_change, this.form.report_result).then((res) => {
            if (res.data == 0) {
              this.$message({
                message: "提交成功！",
                type: "success",
