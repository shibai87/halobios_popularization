<template>
    <div id="knowledge_sort">
        <el-select v-model="options0_value" filterable placeholder="请选择第一级分类" clearable no-match-text="未有相关数据！" no-data-text="空空如也！" @change="getSort(options0_value,0)">
         ...
         <el-select v-model="options1_value" filterable placeholder="请选择第二级分类" clearable no-match-text="未有相关数据！" no-data-text="空空如也！" @change="getSort(options1_value,2)">
        ...
        methods: {
            ...
            getSort(value,step){
                if(step==1){
                    ...
                    Array.from(new Set(this.options1))
                    console.log(this.options1)
                }
                 if(step==2){
                    for (let i = 1; i < this.sorts[0].length; i++) {
                        if(this.sorts[0][i].sort_type_1==value){
                             this.options2.push(this.sorts[0][i].sort_type_1)
                        }
                    }
                    Array.from(new Set(this.options1))
                    console.log(this.options1)
