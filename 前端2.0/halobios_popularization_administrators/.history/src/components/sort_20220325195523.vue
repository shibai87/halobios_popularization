            ...
            getSort(value, step) {
                ...
                if (step == 0) {
                    ...
                    console.log(this.options1)
                     this.geKnowledgeList(this.options1)
                    // console.log(this.options1)
                    this.options1_value = ""
                    this.options2_value = ""
                    this.options3_value = ""
                ...
                if (step == 6) {
                    ...
                    for (let i = 0; i < this.sorts[0].length; i++) {
                       ...
                       this.$store.commit("getSortIdList", this.options6)
                    this.$store.commit("getSortIdListLength",this.options6.length)
                      this.$store.commit("getSortIdListLast", options_id)

                }
            },
// 获取知识列表
   geKnowledgeList(options) {
//                     if(!this.getSortListLength){
//                           this.axios.get("http://localhost:8083/knowledge/get_knowledge?options_id"+this.getSortList)
//                     .then((res)=>{
//                         console.log(res.data[0])
//                     })
//                     }else{
//                         for(let i;i<this.getSortListLength;i++){
//                             let id=this.getSortList[i];
//                             let knowledgeList;
//    this.axios.get("http://localhost:8083/knowledge/get_knowledge?options_id"+id)
//                     .then((res)=>{
//                         console.log(res.data[0])
//                         knowledgeList[i]=res.data[0]
//                     })
//                         }
                        
//                     }
       this.axios.post("http://localhost:8083/knowledge/get_knowledge?options_id",options)
                    .then((res)=>{
                        console.log(res.data[0])
                    })
       ...
       computed: {
                getSortList(){
                    return this.$store.sort_id_list
                },
                 getSortListLength(){
                    return this.$store.sort_id_list_length
                }
        },
        mounted() {
            // 分类功能
