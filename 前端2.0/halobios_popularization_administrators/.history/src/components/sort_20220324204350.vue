                ...
                if(step==0){
                    for (let i = 1; i < this.sorts[0].length; i++) {
                        if(this.sorts[0][i].sort_type_0==value){
                             this.options.push(this.sorts[0][i].sort_type_1)
                        }
                    }
                    this.options1=Array.from(new Set(this.options))
                    console.log(this.options1)
                }
                 if(step==1){
                    for (let i = 1; i < this.sorts[0].length; i++) {
                        if(this.sorts[0][i].sort_type_1==value){
                             this.options.push(this.sorts[0][i].sort_type_2)
                        }
                    }
                    this.options2=Array.from(new Set(this.options))
