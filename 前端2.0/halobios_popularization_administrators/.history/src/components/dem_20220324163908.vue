<template>
        <div id="knowledge_sort">
            <el-select v-model="options0_value" placeholder="请第一级分类">
            ...
            getLoadSortList() {
                this.axios
                    ...
                    .then((res) => {
                        ...
                        // 获取第一级分类
                        this.options0[0].value = this.sorts[0].sort_type_0;
                        for (let i = 1; i < this.sorts.length; i++) {
                            for (let j = 0; j < this.options0.length; j++) {
                                if (this.sorts[i].sort_type_0 != this.options0[j].value && j == this.options0.length -
                                    1) {
                                    this.options0[++j].value = this.sorts[i].sort_type_0;
                                    break;
                                }
                                if (this.sorts[i].sort_type_0 == this.options0[j].value) {
                                    break;
                                }
                            }
                        }
