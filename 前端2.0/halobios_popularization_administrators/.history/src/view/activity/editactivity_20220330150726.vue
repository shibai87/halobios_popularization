...
<script>
  export default {
    ...
    methods: {
      // 获取知识列表+查询列表
      getLoadactivityList() {
        this.axios.get("http://192.168.1.103:8083/activity/get_activity_list?activity_upload=" + 0)
          .then((res) => {
      ...
      delete_activity(activity_id){
        this.axios.get("http://192.168.1.103:8083/activity/delete_activity?activity_id="+activity_id)
          .then((res) => {
      ...
      //视频路径
      Video(row) {
        var url = "http://192.168.1.103:8083/activity/" + row.activity_file_name
        console.log(url)
        return url
      },
    },
