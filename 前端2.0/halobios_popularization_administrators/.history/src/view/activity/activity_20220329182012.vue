      ...
      <ul v-for="(item,index) in activityList" :key="index" class="activity_list">
        <li class="acitivity_item">
            ...
            <div class="activity_operation"> -->
              ...
              <!-- <el-button type="primary" @click="edit_activity(item)">编辑</el-button> -->
              <el-button type="primary" @click="delete_activity(item)">删除</el-button>
            </div>
        </li>
      </ul>
    </div>
  ...
  export default {
    ...
    methods: {
      ...
      edit_activity(activity_id) {
        ...
        this.$router.push("/editactivity_detai");
      },
      delete_activity(item){
        console.log(item,item.activity_id)
