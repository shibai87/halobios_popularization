      ...
      <ul v-for="(item,index) in classList" :key="index" class="class_list">
        <li class="acitivity_item">
            ...
            <div class="class_operation"> 
              ...
              <el-button type="primary" @click="edit_class(item.class_id)">编辑</el-button>
              <el-button type="primary" @click="delete_class(item.class_id)">删除</el-button>
            </div>
        </li>
      </ul>
    </div>
...
<script>
  export default {
    ...
    methods: {
      // 获取知识列表+查询列表
      getLoadclassList() {
        this.axios.get("http://localhost:8083/class/get_class_list?class_upload=" + 1)
          .then((res) => {
      ...
      delete_class(class_id){
        this.axios.get("http://localhost:8083/class/delete_class?class_id="+class_id)
          .then((res) => {
      ...
      //视频路径
      Video(row) {
        var url = "http://localhost:8083/class/" + row.class_file_name
        console.log(url)
        return url
      },
    },
