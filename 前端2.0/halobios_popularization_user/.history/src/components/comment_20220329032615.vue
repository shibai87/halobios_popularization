...
<script>
    ...
    export default {
        ...
        methods: {
            judgeComment(){
                // console.log(this.getUserId,this.getKnowledgeId)
                   this.axios.get(
                    ...
                    .then((res) => {
                        console.log("查询用户是否有该操作")
                        console.log(res,res.data.msg,res.data.data)
                        if (res.data.msg == '用户已有过该操作') {
                            this.my_judge_comment=true
                            this.operation_id=res.data.data
                        } else {
            ...
            deleteComment(){
                this.axios.get("http://192.168.1.103:8083/operation/deleteComment?operation_id=180")
            }
        },
        mounted() {
             this.judgeComment()
