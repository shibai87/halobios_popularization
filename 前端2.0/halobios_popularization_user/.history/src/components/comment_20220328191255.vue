        ...
        <div class="comment_content">
            <van-field v-model="my_message" rows="1" center clearable autosize label="我的留言" type="textarea"
                placeholder="请输入留言"><template #button>
                    <van-button size="small" type="primary" @click="addOpration">提交</van-button>
                </template></van-field>
        </div>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
                ...
                this.axios.get("http://192.168.1.103:8083/operation/getOperationId?operation_user_id=" + this.getUserId +
                    ...
                    .then((res) => {
                        ...
                        if (res.data.msg=='查询成功') {
                            this.operationId=res.data.operation_id
                            this.addComment(res.data.data.operation_id)
                        } else {
                            Dialog.alert({
                                title: '查询失败',
                                message: '查询失败!',
            ...
            addComment(val) {
                ...
                console.log(this.operationId,this.my_message)
