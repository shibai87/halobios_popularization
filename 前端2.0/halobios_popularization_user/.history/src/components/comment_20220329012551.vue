        ...
        <div class="comment_content">
            <van-field v-model="my_message" rows="1" center clearable autosize label="我的留言" type="textarea"
                placeholder="请输入留言"><template #button>
                    <van-button size="small" type="primary" @click="addComment" :v-show="judge_comment">提交</van-button>
                    <van-button size="small" type="primary"  :v-show="judge_comment">删除</van-button>
                </template></van-field>
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            judgeComment(){
                axios.get("http://192.168.1.103:8083/operation/getCommentId?operation_user_id=2&operation_source_type=2&operation_source_id=2&operation_type=2")
                .then((res)=>{
