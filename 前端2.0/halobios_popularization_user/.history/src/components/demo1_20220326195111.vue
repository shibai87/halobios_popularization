  ...
  <div class="keyboard" v-show="showKeyboard" v-clickoutside="closeModal">
    <div v-for="(keys,index) in keyList" :key="index">
      <div v-for="(key,index) in keys" :key="index">
        <i v-if="key === 'top'" @click.stop="clickKey" @touchend.stop="clickKey"
          class="iconfont icon-zhiding tab-top"></i>
        <i v-else-if="key === '123'" @click.stop="clickKey" @touchend.stop="clickKey" class="tab-num">123</i>
        <i v-else-if="key === 'del'" @click.stop="clickKey" @touchend.stop="clickKey"
          class="iconfont icon-delete key-delete"></i>
        <i v-else-if="key === 'blank'" @click.stop="clickKey" @touchend.stop="clickKey"
        ...
        <i v-else-if="key === 'enter'" @click.stop="clickKey" @touchend.stop="clickKey"
  ...
  export default {
    ...
    methods: {
      ...
      clickKey(event) {
        ...
        value && value !== '符' && value !== '·' && value !== '123' ? this.emitValue(value) : this.tabHandle(event
