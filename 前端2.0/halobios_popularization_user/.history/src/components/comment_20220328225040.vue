        ...
        <div class="comment_content">
            <van-field v-model="my_message" rows="1" center clearable autosize label="我的留言" type="textarea"
                placeholder="请输入留言"><template #button>
                    <van-button size="small" type="primary" @click="addComment" v-if="!judge_comment">提交</van-button>
                    <van-button size="small" type="primary" @click="addComment" v-else>删除</van-button>
                </template></van-field>
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            judgeComment(){
                this.axios.get("http://192.168.1.103:8083/operation/getCommentId?operation_user_id="+this.getUserId+"&operation_source_type=0"+"&operation_source_id="+this.getKnowledge+"&operation_type=2")
                .then((res)=>{
