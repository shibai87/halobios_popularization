<template>
  <div id="knowledge">
    <div id="knowledge_top">
      <van-notice-bar color="#1989fa" background="#ecf9ff" left-icon="info-o" speed=50
        text="请通过点击弹出框的确定按钮进行每一级分类的选择！选择完所有分类选项才能进行分类功能！" />
      <Sort></Sort>
        ...
        <van-row>
          ...
          <van-col span="4" offset="3">
            <div class="knowledge_operation">
              <van-button type="primary" @click="knowledge(item.knowledge_id)" size="mini">查看</van-button>
            </div>
          </van-col>
        </van-row>
      </div>
    ...
    <div v-else id="knowledge_list">
      <van-empty image="error" description="未发现该分类下的科普知识" />
    </div>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      knowledge(knowledge_id) {
         localStorage.setItem('getKnowledge',knowledge_id);
