<template>
  <div id="user_infos">
    ...
    <van-field v-model="user_name" label="用户名" placeholder="请输入用户名" class="name"/>
    <van-field v-model="user_tel" type="tel" label="手机号"  placeholder="请输入手机号码"/>
    <van-field v-model="user_pwd" type="password" label="密码" />
    <van-row class="sex">
      ...
      getloadInfo() {
        ...
        this.axios.get("http://192.168.1.103:8083/user/info?user_id=" + this.getUserId)
          .then((res) => {
            ...
            this.info.user_sex = res.data.user_sex
            this.info.user_tel = res.data.user_tel
            this.info.user_pwd = res.data.user_pwd
          })
      },
// 邮箱
 formatter(val) {
