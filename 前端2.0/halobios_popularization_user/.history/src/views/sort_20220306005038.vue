<template>
  <div id="sort">
    <!-- head -->
    <div id="sort_head">
      <!-- <div class="sort" v-for="(sort,index) in sorts" :key="index" @click="change_sort(sort.sort_id)">
        {{sort.sort_type_name}}
      </div> -->
      <div  class="sort">
...
<script>
  export default {
    ...
    methods: {
      change_sort(sort_id) {
        window.alert("选择的分类id为" + sort_id)
      },
      changeSortDetails(knowledge_id){
        window.alert("选择查看的分类详情id为" + knowledge_id),
        this.$router.push('/sort_details')
      }
    },
    mounted() {
...
#sort   .sort_info{
 ...
 #sort  .sort_right_title{
    margin-bottom: 1rem;
  }
 #sort  .sort_right_content{
    display: -webkit-box;
