    ...
    <div id="knowledge">
        <div v-if="!(msg=='查询列表为空')" class="knowledge_list">
            ...
            <van-pull-refresh v-model="isLoading" @refresh="onRefresh">
                <div class="knowledge_list_main" v-if="knowledgeList">
                    <div v-for="(item,index) in knowledgeList" :key="index" class="knowledge_list_details">
                        <van-row>
                            <van-col span="4">
                                <div class="knowledge_img"><img style="width: 70px; height: 80px;"
                                        :src="Image(item.knowledge_img)" />
                                </div>
                            </van-col>
...
//图片路径
const Image = (knowledge_img: any) => Constant.URL + "/knowledge/" + knowledge_img
const onRefresh = () => {
    setTimeout(() => {
        Toast('刷新成功');
...
const getLoadKnowledgeList = () => {
    KnowledgeService.get_knowledge_list(1).then((res: any) => {
        // console.log(res.data)
        knowledgeList.value = res.data
        // console.log(knowledgeList[0])
        msg.value = res.msg
    })
}
// 查询分类下的知识列表
const Knowledge = () => {
    console.log(store.state.sort_id_last)
    if (store.state.sort_id_last) {
        KnowledgeService.get_knowledge_list(1, store.state.sort_id_last)
            .then((res: any) => {
                if (res.data.length) {
                    knowledgeList.value = res.data
                    Dialog.alert({
...
// 重新选择（刷新）
const clear = () => {
    store.commit("getSortIdLast", null)
    store.commit("judgeNewSort", true);
    getLoadKnowledgeList()
}
// 查看科普知识详情
const knowledge = (knowledge_id: any) => {
