<template>
    <div id="knowledge_details">
        <van-nav-bar title="分类详情" fixed left-arrow @click-left="onClickLeft" />
...
const getKonwledgeId = ref<number>(JSON.parse(sessionStorage.getItem('source_id') || ""))
//图片路径
...
// 获取需编辑的知识详情
const getLoadKnowledge = () => {
    KnowledgeService.get_knowledge(getKonwledgeId.value)
        .then((res) => {
...
const getSort = () => { 
    ...
    KnowledgeService.get_sort(ruleForm.value.knowledge_sort_id)
                    .then((res) => {
                     sort.value = res[0] }) }
</script>
