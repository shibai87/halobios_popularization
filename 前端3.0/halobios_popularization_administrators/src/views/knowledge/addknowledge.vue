...
const action = ref(Constant.URL + "/knowledge/newsort")
// 判断是否有图片上传
...
const submitUpload = () => {
    ruleForm.value.knowledge_sort_id = store.state.sort_id_list_last
...
const keepForm = () => {
    ...
    if (store.state.sort_id_list_last) {
        ruleForm.value.knowledge_sort_id = store.state.sort_id_list_last
    } else {
        ruleForm.value.knowledge_sort_id = 0
    }
