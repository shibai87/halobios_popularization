            ...
            <el-form :model="ruleForm" label-width="220px" class="demo-ruleForm">
                ...
                <div v-if="ruleForm.activity_form == `线下参与`">
                    <el-form-item label="活动名称">
                        <el-input v-model="ruleForm.activity_place"/>
                    </el-form-item>
                </div>
...
const action = ref(Constant.URL + "/activity/newactivity")
...
const time=(activity_time)=>{
                ...
                if (time2.getTime() >= time1.getTime()) {
                    ruleForm.value.activity_begin_time = activity_time[0].toLocaleDateString();
                    ruleForm.value.activiy_end_time = activity_time[1].toLocaleDateString();
                } else {
                    ElMessage({
...
const submitUpload = () => {
    ...
    if (ruleForm.value.activity_title && ruleForm.value.activity_info && activity_video_name.value &&
        ruleForm.value.activity_form != null && ruleForm.value.activity_begin_time && ruleForm.value.activiy_end_time) {
            if(ruleForm.value.activity_form== "线上参与" )ruleForm.value.activity_place=null
...
const beforeUpload = (val) => {
    activity_video_name.value = val.name
    console.log(activity_video_name.value);
}
